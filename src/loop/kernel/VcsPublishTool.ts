...
import { parseWriteScope } from '../security/PathSafety.js'
...
export function makeVcsPublishTool(input: {
  ...
  return {
    name: 'vcs_publish',
    description:
      `Stage only the Charter writeScope, commit it, and push the current branch to '${remote}'. ` +
    ...
    async call(raw): Promise<ToolResult> {
      ...
      try {
        ...
        await git(input.projectDir, ['add', '--', ...roots])
        const staged = await gitExit(input.projectDir, ['diff', '--cached', '--quiet', '--', ...roots])
        if (staged === 1) await git(input.projectDir, ['commit', '-m', message, '--', ...roots], 120_000)
