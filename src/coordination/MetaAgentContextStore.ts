...
export class MetaAgentContextStore {
  ...
  private static _cache: { data: MetaAgentSessionContext; ts: number } | null = null
   ...
   */
  private static readonly READ_TIMEOUT_MS = 2_000
   ...
   */
  static async read(): Promise<MetaAgentSessionContext | null> {
    const now = Date.now()
    if (
    ...
    try {
      ...
      const timeoutPromise = new Promise<never>((_, reject) => {
        const t = setTimeout(
          () => reject(new Error(`Context store read timed out after ${MetaAgentContextStore.READ_TIMEOUT_MS} ms`)),
          MetaAgentContextStore.READ_TIMEOUT_MS,
        )
      ...
      const ctx = JSON.parse(raw) as MetaAgentSessionContext
   ...
   */
  static async refresh(summaries: CampaignSummary[]): Promise<void> {
   ...
   *   <contextBlock>
   */
  static async buildInjectionBlock(): Promise<string> {
