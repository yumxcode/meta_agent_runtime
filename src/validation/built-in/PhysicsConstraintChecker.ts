...
const BUILT_IN_CONSTRAINTS: PhysicsConstraint[] = [
  {
...
export class PhysicsConstraintChecker implements VVHook {
  ...
  async run(ctx: VVContext): Promise<VVResult> {
    ...
    return {
      ...
      severity: 'critical',
      message:
        `Physics constraint violated in tool "${ctx.toolName}" ` +
        `(phase: ${ctx.phase}):\n` +
  ...
  private _matches(field: string, patterns: string[]): boolean {
