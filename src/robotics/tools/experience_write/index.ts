...
// ─────────────────────────────────────────────────────────────────────────────
// Flash prompt: extract same-domain abstract principle (experience-only).
//
// v1: experience_write captures an EXPERIENCE and distills its one-line abstract
// principle for same-domain recall matching. It does NOT extract physical
// anchors — anchors are captured independently (session-end strict extraction
// or physical_anchor_write). experience ↔ anchor are decoupled in v1.
// ─────────────────────────────────────────────────────────────────────────────
 ...
 *                     When provided, a flash call extracts the same-domain
 *                     principle at write time (enables recall matching).
 */
  ...
  return {
    ...
    async call(input): Promise<ToolResult> {
      try {
        ...
        // ── Extract abstract principle via flash (best-effort) ────────────
        // Supports same-domain matching in ExperiencePatternChecker / recall.
