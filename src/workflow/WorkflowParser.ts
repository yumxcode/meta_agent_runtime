import type { WorkflowDefinition, WorkflowPhase, GateItem } from './types.js'
...
export class WorkflowParser {
  static parse(raw: string, sourceFile: string): WorkflowDefinition {
    ...
    lines.forEach((line, i) => { if (phaseHeaderRe.test(line)) phaseStarts.push(i) })
    ...
    const phases: WorkflowPhase[] = phaseStarts.map((start, idx) => {
      ...
      for (const line of content.split('\n')) {
        ...
        if (gm) {
          gateItems.push({ id: `${id}_gate_${gateIndex++}`, type: gm[2] as GateItem['type'], description: gm[3].trim(), completed: gm[1] === 'x' })
          continue
        }
        if (/^### Outputs/.test(line)) { inOutputs = true; continue }
        if (/^###/.test(line)) { inOutputs = false; continue }
