...
- `feedback` — the user's corrections or confirmations about your approach (record both). Use the body structure: **Rule:** core rule. **Reason:** why. **Scope:** when it applies.
...
Do NOT use memory for engineering knowledge. Successes/failures, algorithm pitfalls, tuning records, domain facts, computation results, and task state all have dedicated systems — provenance, project docs, the auto-injected context sections, and (when an experience tool such as `experience_write` is available in this session) the ExperienceStore — never memory. Proposals with a disallowed `type` are rejected.
