...
export function getContractPath(contractId: string): string {
...
export class TaskContractStore {
   ...
   */
  static async save(contract: TaskContract): Promise<boolean> {
    try {
   ...
   */
  static async load(contractId: string): Promise<TaskContract | null> {
    const parsed = await readJsonFile<TaskContract>(getContractPath(contractId))
    if (
      !parsed ||
      (parsed as unknown as Record<string, unknown>)['schemaVersion'] !== '1.0' ||
      typeof (parsed as unknown as Record<string, unknown>)['contractId'] !== 'string'
    ) return null
    return parsed
  }
   ...
   */
  static async loadForSession(sessionId: string): Promise<TaskContract | null> {
    try {
  ...
  static async update(
    contractId: string,
    updates: Partial<Omit<TaskContract, 'contractId' | 'sessionId' | 'createdAt' | 'schemaVersion'>>,
   ...
   */
  static async updateCriterionStatus(
    contractId: string,
    criterionId: string,
    ...
    const criteria = existing.acceptanceCriteria.map(c =>
      c.id === criterionId
