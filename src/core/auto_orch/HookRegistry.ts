    ...
    return async (event: PhaseHookEvent): Promise<PhaseHookOutcome> => {
      ...
      for (const h of matched) {
        ...
        if (verdict.action === 'abort') {
          ...
          if (verdict.data?.['failed'] === true) failed = true
        }
