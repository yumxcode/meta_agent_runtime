...
describe('AutoCheckpointStore', () => {
  ...
  it('writes then reads a checkpoint round-trip', () => {
    const ok = writeAutoCheckpoint(ws, {
  ...
  it('returns null on corrupt JSON', () => {
    writeAutoCheckpoint(ws, { schemaVersion: '1.0', sessionId: 's', updatedAt: 1 })
    // Corrupt it
    require('fs').writeFileSync(autoCheckpointPath(ws), '{not json', 'utf-8')
    expect(readAutoCheckpoint(ws)).toBeNull()
  })

  it('update merges: unions completedSteps/artifacts, replaces todos, bumps updatedAt', () => {
    updateAutoCheckpoint(ws, 's1', { completedSteps: ['a'], pendingTodos: ['x', 'y'], artifacts: ['f1'] })
    const cp2 = updateAutoCheckpoint(ws, 's1', {
      completedSteps: ['b'],
      pendingTodos: ['y'],
      artifacts: ['f2'],
      goal: 'g',
  ...
  it('update preserves prior goal when not overridden', () => {
    updateAutoCheckpoint(ws, 's1', { goal: 'original' })
    const cp = updateAutoCheckpoint(ws, 's1', { stopReason: 'max_turns' })
    expect(cp.goal).toBe('original')
    expect(cp.stopReason).toBe('max_turns')
  })

  it('turnCount never regresses (takes the max across updates/resume)', () => {
    updateAutoCheckpoint(ws, 's1', { turnCount: 42 })
    // A resumed run restarts its in-memory counter from 1.
    const cp = updateAutoCheckpoint(ws, 's1', { turnCount: 1 })
    expect(cp.turnCount).toBe(42)
    const cp2 = updateAutoCheckpoint(ws, 's1', { turnCount: 43 })
