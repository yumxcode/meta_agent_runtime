...
function getTaskExecutionRulesSection(): string {
  return `\
## 任务执行规则
...
**全局视角修 bug**：修 bug 不要把视角聚焦在出错的那几行代码，\
应习惯性从全局思考问题——整体设计、数据流与上下游假设。
 ...
 */
function getActionRiskRulesSection(mode: StaticPromptMode): string | null {
...
function getStyleRulesSection(mode: StaticPromptMode): string {
  const base = `\
## 输出风格规则
