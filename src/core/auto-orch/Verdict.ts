/**
 * Verdict — the unified verdict the auto-orch loop and graph both consume.
 *
...
/** A `skipped` verdict — fail-open, but flagged so the host can observe it. */
export function skippedVerdict(note?: string): OrchVerdict {
  return { action: 'continue', skipped: true, note }
}
...
export function fromDrift(v: DriftVerdictLike): OrchVerdict {
  if (v.skipped) return skippedVerdict(v.note)
