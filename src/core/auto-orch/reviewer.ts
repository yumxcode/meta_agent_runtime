...
export async function spawnAndWait(
  ...
  const maxWaitMs = opts?.maxWaitMs ?? 24 * 60 * 1000
...
export async function runReviewer(
  ...
  if (rec?.status !== 'completed' || !rec.result?.summary) {
    return { action: 'done', label: 'pass', skipped: true, note: `${input.role} unavailable`, data: { costUsd: cost } }
  }
  const verdict = parseRoleVerdict(rec.result.summary)
  if (!verdict) {
    return { action: 'done', label: 'pass', skipped: true, note: `${input.role} returned an unparsable verdict`, data: { costUsd: cost } }
  }
