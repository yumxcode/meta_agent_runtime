...
export function createWorkflowStatusTool(definition, getState) {
    return {
        ...
        async call(_input, _ctx) {
            ...
            if (!state)
                return { content: 'No workflow state found. Workflow may not be initialised.', isError: true };
            const phase = definition.phases.find(p => p.id === state.currentPhaseId);
            if (!phase)
                return { content: `Unknown phase: ${state.currentPhaseId}`, isError: true };
