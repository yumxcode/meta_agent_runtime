...
// ─────────────────────────────────────────────────────────────────────────────
export function getMetaAgentCompactPrompt() {
...
// ─────────────────────────────────────────────────────────────────────────────
export async function buildCompactInstructions(rtx, sessionId, sessionStartMs, 
...
taskContract) {
    ...
    if (taskContract) {
        lines.push('', '**Task Contract（目标锚点，严禁修改——必须逐字出现在摘要第 0 章）：**', `  contractId: ${taskContract.contractId}`, `  Primary Goal: ${taskContract.primaryGoal}`);
    ...
    if (snapshot && snapshot.provenanceRecords.length > 0) {
        for (const r of snapshot.provenanceRecords) {
            if (!seenIds.has(r.id)) {
                snapshotLines.push(`  [${r.id}] ${r.toolName}(${r.inputSummary}) → ${r.vv} fidelity=L${r.fidelityLevel}  ` +
    ...
    try {
        ...
        if (ctx && ctx.activeCampaigns.length > 0) {
            campaignLines = ctx.activeCampaigns.map(c => `  Campaign "${c.projectName ?? c.campaignId}" | Phase: ${c.phase}`);
        }
    }
    catch { /* swallow */ }
    if (campaignLines.length === 0 && snapshot && snapshot.activeCampaigns.length > 0) {
        campaignLines = snapshot.activeCampaigns.map(c => `  Campaign "${c.projectName ?? c.campaignId}" | Phase: ${c.phase}  [from snapshot]`);
    }
...
//# sourceMappingURL=compactPrompt.js.map