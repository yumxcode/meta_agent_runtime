...
import { EMPTY_USAGE, accumulateUsage, } from './types.js';
...
import { saveRunStateSnapshot, cleanupRunStateSnapshot, } from './compact/runStateSnapshot.js';
// ─────────────────────────────────────────────────────────────────────────────
...
export class MetaAgentSession {
    ...
    constructor(config = {}) {
        ...
        if (this.config.runtimeContext) {
            ...
            this.toolRegistry = new Map(this.config.tools.map(t => [
                t.name,
    ...
    async *_submitInner(prompt, mode) {
        ...
        while (turnCount < this.config.maxTurns) {
            ...
            try {
                ...
                toolUseCalls = finalMsg.content.filter((b) => b.type === 'tool_use');
                // ── Auto-compact check ────────────────────────────────────────────
                ...
                if (toolUseCalls.length === 0 && shouldCompact(this.config.model, finalMsg.usage.input_tokens)) {
                    ...
                    await saveStateSnapshot(this.sessionId, this.config.runtimeContext, this.sessionStartMs);
                    try {
                        const { newMessages } = await runCompact(this.client, this.config.model, this.mutableMessages, this.sessionId, this.abortController.signal);
                        ...
                        if (this.config.verbose) {
                            console.error(`[MetaAgent:${this.sessionId.slice(0, 8)}] Auto-compact triggered at ` +
                        ...
                        console.error(`[MetaAgent:${this.sessionId.slice(0, 8)}] Auto-compact failed (snapshot preserved for recovery): ` +
            ...
            if (lastStopReason === 'end_turn' || toolUseCalls.length === 0) {
                ...
                void cleanupRunStateSnapshot(this.sessionId, this._taskContract?.contractId).catch(() => { });
                yield {
                    type: 'result',
                    subtype: 'success',
                    sessionId: this.sessionId,
            ...
            for (const batch of batches) {
                if (batch.concurrent) {
                    ...
                    const settled = await Promise.allSettled(batch.calls.map(async (tc) => ({ tc, result: await this.callTool(tc) })));
                    for (const outcome of settled) {
                        const { tc, result } = outcome.status === 'fulfilled'
                            ? outcome.value
                            : {
                    ...
                    for (const tc of batch.calls) {
                        ...
                        if (this._planModeRef.active) {
                            ...
                            if (askUserFn) {
                                ...
                                const answer = await askUserFn(`[Plan Mode] Allow tool "${tc.name}"?\n${inputStr}`, ['yes', 'no']);
    ...
    async callTool(tc) {
        ...
        if (this.config.beforeToolCall) {
            const guard = await this.config.beforeToolCall(tc.name, tc.input);
            if (guard.action === 'deny') {
                return {
                    content: `[操作已拒绝] ${guard.reason ?? '用户拒绝了此操作。'} ` +
