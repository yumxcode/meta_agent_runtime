...
import { buildMemoryGuidanceLines, ensureMemoryDirExists, loadMemoryIndex, } from './memory/memdir.js';
...
// ─────────────────────────────────────────────────────────────────────────────
// D1a — Memory Guidance  [memoized]
//
// Static taxonomy + write protocol text.  Never changes within a session.
// Cached once so prompt-cache prefix is stable across turns.
// ─────────────────────────────────────────────────────────────────────────────
export function buildMemoryGuidanceSection() {
...
// ─────────────────────────────────────────────────────────────────────────────
export function buildMemoryContentSection(currentQuery, client, sessionMode) {
        ...
        if (relevant.length > 0) {
            ...
            for (const mem of relevant) {
                ...
                if (header.confidence === 'low')
                    metaParts.push('⚠ confidence:low');
                else if (header.confidence === 'medium')
                    metaParts.push('confidence:medium');
...
// ─────────────────────────────────────────────────────────────────────────────
export function buildEnvInfoSection(sessionId, sessionStartMs, tools) {
...
//
// Injected when mode !== 'direct'.  Direct-mode sessions (quick Q&A) do not
// need sig-fig or dimensional-consistency rules; agentic and campaign sessions do.
// ─────────────────────────────────────────────────────────────────────────────
