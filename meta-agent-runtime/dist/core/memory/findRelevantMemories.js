...
const MEMORY_CONFIDENCES = new Set([
    'high', 'medium', 'low',
]);
// ─────────────────────────────────────────────────────────────────────────────
// Frontmatter parser
...
function _passesFilters(header, opts, nowDate) {
    ...
    if (scope === 'project' && opts.projectScope) {
        ...
        if (tag && tag !== opts.projectScope)
            return false;
    }
    if (scope === 'campaign' && opts.campaignScope) {
        const tag = header.campaign ?? '';
        if (tag && tag !== opts.campaignScope)
            return false;
    }
