...
export function getSnapshotPath(sessionId) {
 ...
 */
export function saveStateSnapshot(sessionId, rtx, sessionStartMs) {
        ...
        try {
            ...
            try {
                ...
                if (ctx?.activeCampaigns) {
                    ...
                    const enriched = await Promise.all(ctx.activeCampaigns.map(async (c) => {
                        const base = {
                        ...
                        try {
                            ...
                            base.objectives = objectives.map(o => `${o.direction} ${o.name}${o.unit ? ` (${o.unit})` : ''}`);
                            base.constraints = constraints.map(ct => `${ct.name}: ${ct.expression} (${ct.type})`);
                        }
 ...
 */
export async function loadStateSnapshot(sessionId) {
    try {
        const path = getSnapshotPath(sessionId);
 ...
 */
export async function cleanupStateSnapshot(sessionId) {
