 ...
 */
export function makeDesignPoint(variables) {
...
export class DOESampler {
     ...
     */
    static lhs(space, n, seed = 42) {
        if (n <= 0)
            return [];
        const rng = new SeededRandom(seed);
        const vars = space.variables;
        // Step 1+2: for each variable build N stratum samples
        const strata = vars.map(() => Array.from({ length: n }, (_, i) => (i + rng.next()) / n));
     ...
     */
    static grid(space, levelsPerVar) {
        if (levelsPerVar <= 0)
            return [];
        const vars = space.variables;
        // Build level list per variable
        const levels = vars.map(v => {
            if (v.type === 'discrete' || v.type === 'categorical') {
                const vals = v.values ?? [];
                if (vals.length <= levelsPerVar)
                    return vals;
                ...
                return Array.from({ length: levelsPerVar }, (_, i) => vals[Math.round(i * step)]);
            }
            const [lo, hi] = v.bounds ?? [0, 1];
            if (levelsPerVar === 1)
                return [v.type === 'integer' ? Math.round((lo + hi) / 2) : (lo + hi) / 2];
     ...
     */
    static refine(space, seedPoints, pointsPerSeed, radius = 0.1, seed = 99) {
        if (seedPoints.length === 0 || pointsPerSeed <= 0)
            return [];
        const rng = new SeededRandom(seed);
        const vars = space.variables;
        const results = [];
...
//# sourceMappingURL=DOESampler.js.map