...
} as const satisfies Record<string, DimensionVector>
...
const DIM_SYMBOL: Record<BaseDimension, string> = {
  mass:        'M',
  length:      'L',
  time:        'T',
  temperature: 'Θ',
 ...
 *   {}                                  →  "dimensionless"
 */
export function formatDimension(dv: DimensionVector): string {
  const parts: string[] = []
 ...
 */
export function dimensionsMatch(a: DimensionVector, b: DimensionVector): boolean {
  for (const dim of BASE_DIMENSIONS) {
    const ea = a[dim] ?? 0
    const eb = b[dim] ?? 0
...
export function multiplyDimensions(a: DimensionVector, b: DimensionVector): DimensionVector {
  ...
  for (const dim of BASE_DIMENSIONS) {
    ...
    if (exp !== 0) (result as any)[dim] = exp
  }
  return result
}
 ...
 */
export function invertDimension(dv: DimensionVector): DimensionVector {
  const result: DimensionVector = {}
  for (const dim of BASE_DIMENSIONS) {
    const exp = dv[dim] ?? 0
    if (exp !== 0) (result as any)[dim] = -exp
 ...
 */
export function identifyDimension(dv: DimensionVector): string | null {
  for (const [name, ref] of Object.entries(DIMENSIONS)) {
    if (dimensionsMatch(dv, ref)) return name
  }
  return null
}
