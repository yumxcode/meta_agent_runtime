...
import type { PhysicalQuantity, DimensionVector, DimensionError, DimensionSpec } from './types.js'
import { dimensionsMatch, formatDimension } from './dimensions.js'
import { defaultRegistry, UnitRegistry } from './UnitRegistry.js'
...
function isPhysicalQuantity(v: unknown): v is PhysicalQuantity {
...
export class DimensionalConsistencyChecker {
  ...
  constructor(registry: UnitRegistry = defaultRegistry) {
  ...
  private _checkRecord(
    ...
    for (const [field, fieldSpec] of Object.entries(spec)) {
      if (!fieldSpec.dimension) continue  // no dimension declared → skip
      ...
      if (!dimensionsMatch(value.dimension, fieldSpec.dimension)) {
        errors.push({
          ...
          hint:
            `Field "${field}": expected dimension ${formatDimension(fieldSpec.dimension)} ` +
  ...
  scanForQuantities(
    record: Record<string, unknown>,
  ): Array<{ field: string; qty: PhysicalQuantity; unitKnown: boolean; consistent: boolean; hint?: string }> {
    const results = []
  ...
  convert(qty: PhysicalQuantity, targetUnit: string): PhysicalQuantity {
    ...
    throw new Error(
      `Cannot convert from "${qty.unit}" (${formatDimension(srcDef.dimension)}) ` +
  ...
  private _siUnitFor(dv: DimensionVector): string | null {
    const SI_UNITS: Array<[DimensionVector, string]> = [
      [{ mass: 1 },                                           'kg'],
      [{ length: 1 },                                         'm'],
      [{ time: 1 },                                           's'],
      [{ temperature: 1 },                                    'K'],
