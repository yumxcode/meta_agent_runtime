...
function isRgAvailable(): boolean {
  ...
  try { execFileSync('rg', ['--version'], { timeout: 2000 }); _rgAvailable = true } catch { _rgAvailable = false }
  return _rgAvailable
}
    ...
    async call(input: Record<string, unknown>, _ctx: ToolCallContext): Promise<ToolResult> {
        ...
        try {
          for (const entry of readdirSync(dir, { withFileTypes: true })) {
            ...
            if (entry.isDirectory()) {
              ...
              try { if (regex.test(readFileSync(full, 'utf-8'))) matchedFiles.push(full) } catch { /* skip */ }
            }
          }
        } catch { /* skip */ }
      }
      try {
        if (statSync(searchPath).isFile()) {
          if (regex.test(readFileSync(searchPath, 'utf-8'))) matchedFiles.push(searchPath)
