...
export async function createReadMcpResourceTool(): Promise<MetaAgentTool> {
  ...
  return {
    ...
    async call(input: Record<string, unknown>, _ctx: ToolCallContext): Promise<ToolResult> {
      ...
      if (!client) return { content: `MCP server "${serverName}" not found`, isError: true }
      if (!client.readResource) return { content: `Server "${serverName}" does not support reading resources`, isError: true }
      try {
