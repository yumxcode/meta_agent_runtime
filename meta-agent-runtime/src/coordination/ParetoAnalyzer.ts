...
export class ParetoAnalyzer {
  ...
  analyze(results: EvaluationResult[]): ParetoFront {
    ...
    const dominated = new Array<number>(n).fill(0)   // domination count
    const dominates = Array.from({ length: n }, () => [] as number[])
    ...
    // Build rank layers
    const allRanks: EvaluationResult[][] = []
    let currentFront: number[] = []

    for (let i = 0; i < n; i++) {
      if (dominated[i] === 0) currentFront.push(i)
    }
   ...
   */
  private _dominanceRelation(a: number[], b: number[]): 0 | 1 | -1 {
    let aWins = false
    let bWins = false
    for (let i = 0; i < a.length; i++) {
      if (a[i]! < b[i]!) aWins = true
      else if (a[i]! > b[i]!) bWins = true
    }
   ...
   */
  private _hypervolume2D(front: EvaluationResult[]): number {
