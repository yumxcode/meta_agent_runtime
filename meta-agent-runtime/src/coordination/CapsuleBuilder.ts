...
function buildStructuredData(
  store: CampaignStateStore,
  front: ParetoFront | null,
): CampaignContextCapsule['structuredData'] {
  const totalPoints = store.sampledPoints.length
  ...
  const bestResults: Record<string, { value: number; unit: string; pointId: string }> = {}
  ...
  const estimatedMinutesRemaining =
    store.pendingTaskCount > 0 ? estimateMinutes(store) : null
...
function findBestForObjective(
  ...
  return results.reduce((best, r) => {
    ...
    if (val === undefined || bestVal === undefined) return best
    return obj.direction === 'minimize' ? (val < bestVal ? r : best) : (val > bestVal ? r : best)
  })
}

function formatTopVars(result: EvaluationResult, n: number): string {
...
function statusIcon(phase: CampaignPhase): string {
  if (phase === 'DONE') return '✅'
  if (phase === 'FAILED') return '❌'
  if (USER_CHECKPOINT_PHASES.has(phase)) return '🎯'
  return '⏳'
}

function callToAction(phase: CampaignPhase, front: ParetoFront | null): string | null {
  switch (phase) {
    case 'EVALUATING_L0':
    case 'ESCALATING_L1':
    case 'ESCALATING_L2':
...
function suggestNextAction(phase: CampaignPhase, front: ParetoFront | null): string {
...
function estimateMinutes(store: CampaignStateStore): number {
  const pending = store.pendingTaskCount
  const phase = store.phase
  const minutesPerPoint =
    phase === 'EVALUATING_L0' ? 2 :
