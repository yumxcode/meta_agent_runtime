...
export class ProvenanceTracker {
  ...
  private cache = new Map<ProvenanceId, ProvenanceRecord>()
   ...
   */
  async record(input: ProvenanceInput): Promise<ProvenanceId> {
  ...
  async get(id: ProvenanceId): Promise<ProvenanceRecord | null> {
   ...
   */
  async chain(id: ProvenanceId): Promise<ProvenanceRecord[]> {
    const visited = new Set<ProvenanceId>()
    const result: ProvenanceRecord[] = []

    let current: ProvenanceRecord | null = await this.get(id)
    while (current) {
   ...
   */
  async findByInputHash(inputHash: string): Promise<ProvenanceRecord[]> {
    await this._ensureCacheLoaded()
   ...
   */
  async findDuplicate(input: unknown): Promise<ProvenanceRecord | null> {
   ...
   */
  async summary(id: ProvenanceId): Promise<string> {
  ...
  private async _save(rec: ProvenanceRecord): Promise<void> {
  ...
  private async _ensureCacheLoaded(): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      const id = entry.replace(/\.json$/, '') as ProvenanceId
      if (this.cache.has(id)) continue         // already in cache from get()
      try {
  ...
  private _matches(r: ProvenanceRecord, f: ProvenanceFilter): boolean {
    ...
    if (f.since !== undefined && r.timestamp < f.since) return false
    if (f.until !== undefined && r.timestamp > f.until) return false
    return true
  }
}
