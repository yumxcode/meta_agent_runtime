...
export const BUILT_IN_OOM_DB: OOMReferenceDB = {
  '*': {
    // Stress / pressure
...
export class OOMChecker implements VVHook {
  readonly name = 'OOMChecker'
  readonly phase = 'post_call' as const
  readonly appliesTo = '*' as const

  private readonly db: OOMReferenceDB
  ...
  async run(ctx: VVContext): Promise<VVResult> {
    ...
    for (const [field, rawValue] of Object.entries(ctx.output)) {
      ...
      if (value === null) continue  // skip non-numeric fields

      const range = this._findRange(ctx.toolName, field)
      if (!range) continue          // no reference data → skip (no false positives)

      if (!this._inRange(value, range)) {
        findings.push(
          `Field "${field}" = ${this._fmt(value)} ${range.unit} ` +
  ...
  private _findRange(toolName: string, field: string): OOMRange | null {
  ...
  private _inRange(value: number, range: OOMRange): boolean {
  ...
  private _extractNumber(v: unknown): number | null {
