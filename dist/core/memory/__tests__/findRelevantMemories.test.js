...
// Helpers
// ─────────────────────────────────────────────────────────────────────────────
function feedbackFrontmatter(name) {
    return `---\nname: ${name}\ndescription: ${name} feedback\ntype: feedback\ndate: 2025-01-01\n---\n\n**Rule:** ${name}`;
}
function userFrontmatter(name) {
    return `---\nname: ${name}\ndescription: ${name} user profile\ntype: user\ndate: 2025-01-01\n---\n\n${name} background`;
}
async function writeFeedbackFile(dir, name, mtimeMsOffset) {
...
// ─────────────────────────────────────────────────────────────────────────────
// Session-mode filtering — campaign_lessons
// ─────────────────────────────────────────────────────────────────────────────
describe('findRelevantMemories — session mode filtering', () => {
    it('excludes campaign_lessons when sessionMode is not campaign', async () => {
        const dir = await tempMemoryDir();
        const path = join(dir, 'battery_threshold.md');
        await writeFile(path, '---\nname: battery threshold\ndescription: DOE threshold\ntype: campaign_lessons\ndate: 2025-01-01\n---\n\nThreshold data', 'utf-8');
    ...
    it('includes campaign_lessons when sessionMode is campaign', async () => {
        const dir = await tempMemoryDir();
        const path = join(dir, 'battery_threshold.md');
        await writeFile(path, '---\nname: battery threshold\ndescription: DOE threshold\ntype: campaign_lessons\ndate: 2025-01-01\n---\n\nThreshold data', 'utf-8');
        // campaign mode — should load campaign_lessons (requires flash client; test keyword fallback)
        const results = await findRelevantMemories({ query: 'battery threshold', memoryDir: dir, sessionMode: 'campaign' });
        // campaign_lessons is a candidate (not always-relevant), requires flash or keyword match
        const campResults = results.filter(r => r.header.type === 'campaign_lessons');
        expect(campResults).toHaveLength(1);
    });
});
//# sourceMappingURL=findRelevantMemories.test.js.map