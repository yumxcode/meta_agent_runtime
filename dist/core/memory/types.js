...
export const TYPES_SECTION = [
    ...
    // ── robot_lessons ────────────────────────────────────────────────────────
    '<type>',
    '  <name>robot_lessons</name>',
    '  <description>Robotics mode 中跨 session 可复用的错误、警告和避坑经验。记录以后不要再犯什么，以及遇到类似机器人、硬件或算法条件时要先检查什么。成熟、系统化、可检索的工程经验应进入 ExperienceStore，不进入 memory。</description>',
    '  <when_to_save>当 robotics session 中出现可泛化的失败模式、危险操作、硬件约束误判、仿真/实机差异警告，且未来 robotics session 都应受益时保存。</when_to_save>',
    '  <body_structure>**警告/错误：** 核心模式。**触发条件：** 何时容易发生。**以后做法：** 避免或验证步骤。**适用范围：** 机器人类型、硬件或算法边界。</body_structure>',
    '  <examples>',
    '    user: 以后对这个机械臂不要默认关节 3 可以连续旋转，上次这样导致轨迹规划不可执行。',
    '    → [保存 robot_lessons 记忆：该类机械臂的 J3 不是连续旋转关节；规划前必须读取 hardware profile 验证关节限制。]',
    '  </examples>',
    '</type>',
    // ── reference ─────────────────────────────────────────────────────────────
    '<type>',
...
export const WHAT_NOT_TO_SAVE_SECTION = [
    '## 不应存入记忆的内容（按 mode 区分）',
    '',
    '**Campaign mode 三条硬边界——这些有专用系统，不得绕过：**',
    '',
    '1. **仿真/计算结果**（特定输入→特定输出）',
    ...
    '**Robotics mode 硬边界：**',
    '',
    '成熟、系统化、可检索的工程经验不写入 memory，写入 **ExperienceStore**。',
    'memory 只保存跨 robotics session 都有价值的公共偏好、错误模式、警告和外部资源指针。',
    '',
    '同样不应存储：',
    '- 临时会话上下文或对话摘要',
...
export const WHEN_TO_ACCESS_SECTION = [
    ...
    '- **robotics 任务开始前**：先检查 robot_lessons、user/feedback 和相关 reference；成熟工程经验从 ExperienceStore 加载。',
...
export const TRUSTING_RECALL_SECTION = [
    ...
    'robot_lessons 是错误/警告记忆，不替代 ExperienceStore 中的成熟工程经验。',
    '',
];
// ─────────────────────────────────────────────────────────────────────────────
// 工程记忆漂移警告（比 CC 更强）
...
export const DRIFT_CAVEAT = [
    ...
    '  不得在不同工程领域之间盲目迁移阈值。',
    '- **robot_lessons 警告**：仅用于提醒检查风险；成熟流程、算法经验和调参方法应从 ExperienceStore 获取。',
...
export const SCOPE_FRESHNESS_SECTION = [
    ...
    '',
    '**重要：** `scope` 和 `domain` 字段用于排除跨领域的不相关记忆，防止旧上下文污染当前推理。',
    '',
];
