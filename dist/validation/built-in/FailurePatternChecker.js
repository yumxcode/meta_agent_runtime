...
Abstract Principle: ${e.abstractPrinciple}` +
        ...
        (e.workarounds?.length ? `\nWorkaround: ${e.workarounds[0]}` : '')).join('\n\n');
}
...
function formatSlotContent(e) {
    ...
    if (e.failureReason)
        lines.push(`**Failure detail:** ${e.failureReason}`);
...
// ─────────────────────────────────────────────────────────────────────────────
export class ExperiencePatternChecker {
    source;
    flash;
    pager;
...
//# sourceMappingURL=FailurePatternChecker.js.map