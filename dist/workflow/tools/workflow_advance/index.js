...
export function createWorkflowAdvanceTool(projectDir, definition, onStateChange) {
    return {
        ...
        async call(input, ctx) {
            ...
            if (!state)
                return { content: 'No workflow state. Initialise workflow first.', isError: true };
            ...
            if (check.needsApproval.length > 0 && !input['confirmed']) {
                if (ctx.askUser) {
                    ...
                    const answer = await ctx.askUser(`Advancing to next phase requires your approval:\n\n${approvalList}\n\nDo you confirm?`, ['Yes, advance to next phase', 'No, not yet']);
