...
export async function createRunAgentTool(bridge) {
    ...
    return {
        ...
        async call(input, ctx) {
            ...
            if (!taskDescription)
                return { content: 'Error: task_description is required', isError: true };
            ...
            try {
                ...
                while (Date.now() - startMs < MAX_WAIT_MS) {
                    ...
                    if (!status)
                        return { content: `Internal error: task ${record.taskId} not found`, isError: true };
                    ...
                    if (status.status === 'failed')
                        return { content: `Sub-agent failed: ${status.result?.error ?? 'unknown error'}`, isError: true };
                    if (status.status === 'cancelled')
                        return { content: 'Sub-agent was cancelled', isError: true };
                }
