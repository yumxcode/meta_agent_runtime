...
import { STALE_CLAIM_MS, VALID_TASK_STATUSES, isActiveTask, isStaleClaim, } from './types.js';
import { renderBoard, renderGoals, renderLog, renderReadme, } from './render.js';
const execFileAsync = promisify(execFile);
export { STALE_CLAIM_MS, VALID_TASK_STATUSES, isActiveTask, isStaleClaim, };
const TEAM_DIR = 'team';
...
export class TeamStore {
    ...
    get teamDir() { return join(this.projectDir, TEAM_DIR); }
    get statePath() { return join(this.teamDir, STATE_FILE); }
    msSinceLastFetch() {
    ...
    async note(input) {
        ...
        if (!direction)
            throw new Error('note direction is required');
        if (!outcome)
            throw new Error('note outcome is required');
        const state = await this.ensure();
    ...
    async formatPromptContext() {
        ...
        for (const t of state.tasks) {
            for (const a of t.attempts.slice(-3))
                recentAttempts.push({ task: t, attempt: a });
        }
    ...
    formatOwnedError(task) {
        const ago = task.claimedAt
            ...
            : '时间未知';
        return (`${task.id} 已被 ${task.ownerUnit} 领取（${ago}）。` +
