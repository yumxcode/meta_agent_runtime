 ...
 */
export function buildR1Section(robot, getMode) {
...
export function buildR3Section(bridge, gitMgr, getState) {
    return DANGEROUS_uncachedSystemPromptSection('robotics_subagents', async () => {
        ...
        const rows = await Promise.all(activeTasks.map(async (task) => {
            ...
            if (task.branchName && gitMgr.enabled) {
                try {
                    const bs = await gitMgr.getTaskBranchStatus(task.taskId, task.branchName);
...
// ── R4 — Hardware Profile ─────────────────────────────────────────────────────
export function buildR4Section(hwProfile, robot) {
...
export function buildR5Section(getState, resumedAt) {
    return DANGEROUS_uncachedSystemPromptSection('robotics_progress', () => {
        const state = getState();
        if (!state)
            return null;
...
//# sourceMappingURL=dynamicSections.js.map