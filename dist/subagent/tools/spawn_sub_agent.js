 ...
 */
export function makeSpawnSubAgentTool(bridge) {
    return {
...
- Tasks that must complete before ANY other action`,
        inputSchema: {
            ...
            properties: {
                ...
                sandbox: {
                    type: 'object',
                    description: '(Optional) OS-level sandbox policy for bash commands in this sub-agent. ' +
                    ...
                    properties: {
                        write_allow_paths: {
                            type: 'array',
                            items: { type: 'string' },
                            description: 'Extra absolute paths the sub-agent may write to, in addition to its workspace root. ' +
                                'Example: ["/tmp/artifacts", "/data/output"]',
                        },
                        read_deny_paths: {
                            type: 'array',
                            items: { type: 'string' },
                            description: 'Absolute paths the sub-agent may NOT read. ' +
                        ...
                        network: {
                            ...
                            description: '"none" — unshare the network namespace (no outbound connections). ' +
                                '"unrestricted" — inherit parent network access (default).',
                        },
                        command_timeout_ms: {
                            type: 'number',
                            description: 'Per-command timeout override in ms (max 120000). ' +
        ...
        async call(input, ctx) {
            ...
            try {
                ...
                const sandboxConfig = sandboxInput
                    ? {
                        writeAllowPaths: sandboxInput['write_allow_paths'],
                        readDenyPaths: sandboxInput['read_deny_paths'],
...
//# sourceMappingURL=spawn_sub_agent.js.map