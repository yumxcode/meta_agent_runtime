...
vi.mock('../SubAgentTaskStore.js', () => ({
    readTask: vi.fn(async (taskId) => mockState.tasks.get(taskId) ?? null),
    writeTask: vi.fn(async (record) => {
        mockState.tasks.set(record.taskId, { ...record });
    }),
    releaseWriteChain: vi.fn(async () => { }),
    listTasksForSession: vi.fn(async (parentSessionId) => [...mockState.tasks.values()].filter(record => record.parentSessionId === parentSessionId)),
}));
...
function completeTask(taskId, costUsd = 0) {
...
describe('SubAgentBridge scheduler', () => {
    ...
    it('rejects spawns when running plus queued capacity is exhausted', async () => {
        ...
        await expect(bridge.spawnSubAgent({ config: { taskDescription: 'third' } })).rejects.toThrow(/queue is full/);
    });
...
//# sourceMappingURL=SubAgentBridge.test.js.map