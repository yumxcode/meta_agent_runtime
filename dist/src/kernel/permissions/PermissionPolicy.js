...
function readPermissionConfig(path) {
    if (!existsSync(path))
        return {};
    try {
...
function mergePermissionDeclaration(base = {}, override = {}) {
    return {
        ...base,
        ...override,
...
function mergePermissionConfig(base, override) {
...
function resolveForPolicy(path, workspaceRoot) {
...
function findWorkspaceViolation(tool, input, workspaceRoot, permission, allowTmp) {
    ...
    if (toolName === 'bash') {
        ...
        while ((match = absPathPattern.exec(command)) !== null) {
            ...
            if (!(allowTmp && (candidate.startsWith('/tmp/') || candidate.startsWith('/var/tmp/'))) &&
...
export function createPermissionPolicy(options = {}) {
    ...
    return async (tool, input, _assistantMessageUuid, _toolUseId, context) => {
        ...
        if (workspaceRoot && !allowOutsideWorkspace && permission.requiresWorkspace !== false) {
            ...
            if (violation)
                return { behavior: 'deny', reason: violation };
        }
        const sensitiveLabel = tool.name === 'bash' ? detectSensitiveBash(record) : null;
        if (options.beforeToolCall && (sensitiveLabel || permission.sensitive === true)) {
            const guard = await options.beforeToolCall(tool.name, record);
