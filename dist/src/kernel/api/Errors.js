...
export function isMaxOutputTokensStopReason(stopReason) {
...
export function isOverloadedError(error) {
    if (error && typeof error === 'object') {
        ...
        if (e['status'] === 529)
            return true;
        if (typeof e['message'] === 'string' && e['message'].toLowerCase().includes('overloaded'))
            return true;
    }
    return false;
}
 ...
 */
export function isFallbackTriggeredError(error) {
...
export const PROMPT_TOO_LONG_ERROR_MESSAGE = "I'm sorry, but my context window is full and I can't continue this conversation. " +
