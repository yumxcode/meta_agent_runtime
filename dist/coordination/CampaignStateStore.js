...
import { appendFile, open, readFile, readdir, stat, writeFile, } from 'fs/promises';
...
export function makeCampaignId(projectName) {
...
export class CampaignStateStore {
    ...
    _withLock(fn) {
        ...
        void run.then(() => CampaignStateStore._releaseLock(campaignId), () => CampaignStateStore._releaseLock(campaignId));
        return run;
    }
    ...
    static async load(campaignId) {
        const state = await readJsonFile(join(campaignDir(campaignId), 'state.json'));
        if (!state)
            throw new Error(`Campaign "${campaignId}" not found or corrupt`);
        ...
        for (const store of all) {
            ...
            if (thresholdMs !== null) {
                ...
                if (ageMs > thresholdMs) {
                    ...
                    try {
                        await store.markFailed(`Abandoned: no progress for ${ageH} h ` +
    ...
    static async listAll() {
        const all = await CampaignStateStore._loadAll();
        return all.sort((a, b) => new Date(b._state.createdAt).getTime() -
    ...
    isCurrentPhaseComplete() {
        const total = this._state.pendingTaskIds.length +
    ...
    async completeTask(taskId) {
        return this._withLock(async () => {
            await this.reload();
            this._state.pendingTaskIds = this._state.pendingTaskIds.filter(id => id !== taskId);
    ...
    async getEvaluations(filter) {
        ...
        return allResults.filter(r => {
            if (filter.feasibleOnly && !r.feasible)
                return false;
            if (filter.fidelity !== undefined && r.fidelity !== filter.fidelity)
                return false;
            return true;
        });
    }
    ...
    async transitionPhase(to) {
        return this._withLock(async () => {
            ...
            if (!valid.includes(to)) {
                throw new Error(`Invalid phase transition: ${from} → ${to}. Valid: ${valid.join(', ')}`);
            }
...
//# sourceMappingURL=CampaignStateStore.js.map