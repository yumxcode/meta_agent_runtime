...
- ✅ 26 条单测全绿（含 9 条 Planner）；`src/kernel` 310 条既有单测零回归

**未开发（标 TODO，后续 PR）**
1. **实盘 NodeRunner**：把 `NodeRunner.run` 接到 `ISubAgentDispatcher.spawnSubAgent`（executor → isolated_write 分支 + 事后 merge；role → 只读快照），并把每节点 `hooks` 装配进该子 session 的 `KernelConfig.phaseHooks`。
2. **AgenticBackendFactory 装配**：`auto-orch` 时构造 Planner + 默认 HookRegistry 并注入 `phaseHooks`，串起 Planner→PlanRunner→NodeRunner 端到端链路；把 drift/verify 也改写成注册到 registry 的角色（替掉两个固定槽）。
3. **Blackboard**：节点间共享通道（按 plan_id/task_id 寻址），支撑真正的兄弟间并行协作（见 auto-mode 讨论中「当前只有父子无兄弟」一项）。
4. **闭环重规划**：在结构边界回调 Planner 改图，受 `replan-depth` 上限约束。
