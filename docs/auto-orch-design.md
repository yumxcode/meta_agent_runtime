...
- ✅ 48 条单测全绿（B+C+Planner+端到端+RoleCatalog+ModeProfiles）；`src/kernel`/`routing`/`modes`/`core` 共 534 条既有单测零回归
...
2. **Blackboard**：节点间共享通道（按 plan_id/task_id 寻址），让 role 的纠偏 messages 真正流入下一个 executor，支撑兄弟间并行协作。
3. **闭环重规划**：在结构边界回调 Planner 改图，受 `replan-depth` 上限约束。
