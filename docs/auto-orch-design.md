...
- ✅ 42 条单测全绿（B+C+Planner+端到端+ModeProfiles）；`src/kernel`/`routing`/`modes`/`core` 既有单测零回归
...
2. **drift/verify → 角色注册表**：把现有两个固定 gate 槽改写成注册到 `HookRegistry` 的角色，与 auto-orch 角色统一。
3. **Blackboard**：节点间共享通道（按 plan_id/task_id 寻址），让 role 的纠偏 messages 真正流入下一个 executor，支撑兄弟间并行协作。
