...
- `validatePlan`：唯一 id、entry 存在、边引用合法、谓词合法、写节点隔离、**优雅终止检测** —— **非法计划绝不执行**。

#### 4.1a 优雅终止检测（`detectUnterminableCycles`）

**两层终止保证**：硬终止由 `bounds` 绝对保证（撞上限即停）；**优雅终止**（跑到 `completed` 而非撞上限）由本检测**静态保证**。
用 Tarjan 求强连通分量，对每个环要求存在「可逃逸节点」：
- (a) 有一条**可达**出边离开环（未被更早的无条件 `always` 边遮蔽），或
- (b) 该节点**没有 `always` 边** —— 于是某个 verdict 不匹配任何出边而自然终止。

健全性：canonical 的 `gen→verify→fix` 经 (b) 通过（verify 只有条件 `fail` 回边，`pass` 即终止），不会被误杀；
而 `A↔B` 全 `always` 的死环、或被 `always` 边遮蔽逃逸边的图会被拒。**被拒的错误信息回灌给 Planner 要求重产**（见 §6 Planner 重试）。
...
内存、单次运行（**不是**持久 checkpoint）。保留完整 post 日志供观测。`correctiveRounds()` 写进 controller 摘要。

**按目标寻址（节点 id 粒度）**：每条 entry 带可选 `to`（目标节点 id）。
- **地址由编排器从拓扑推导**：`PlanRunner` 在节点 verdict 带 `messages` 时，把纠偏投递到 `selectNext` 得出的**那条边的目标节点**（如 verify `fail` 回边指向的 executor）。**agent 完全不需要知道目标 id**，保持地址无关、低耦合（posting 从 KernelNodeRunner 移到了 PlanRunner）。
- **读取按自身 id**：`KernelNodeRunner.runExecutor` 用 `takeCorrectivePrefaceFor(node.id)` 只取**寄给自己**的纠偏 → 解决多 executor 时「B 的反馈被 C 误取」的串味问题，也是并行的前置。
- **按 kind 区分消费语义**：`'corrective'` **取走即清**（恰好一次）；非 corrective（`'output'/'note'`）**持久、非消费式** `readFor(nodeId, kind?)`，支持扇入（多个下游读同一产出）。`to` 缺省＝广播，兼容旧的「谁先读谁取」。
...
- ✅ **优雅终止检测 + Planner 重试**：`detectUnterminableCycles`（Tarjan SCC + 逃逸节点）接入 `validatePlan`，死环静态拒绝；`makeAutoOrchPlanner` 校验失败把错误回灌给 planner 子 Agent 要求重产（`maxAttempts` 默认 2），耗尽才 fail-open。
- ✅ **并行执行**（`kind:'parallel'`）：扇出-汇合即一节点（单游标不变）；L1 不相交写域（静态校验 + 软约束）、L2 `planMerge` 改动交集预检、L3c integrator 角色冲突解析；`BranchOps` 抽象（纯逻辑 stub 全测）+ `KernelBranchOps` 真实适配（worktree 事务化合并，安全降级）。
- ✅ 75 条 auto-orch 单测全绿；`src/core`/`routing` 257 条 + `src/kernel`/`modes` 既有单测零回归

### 4.7 并行执行（`kind:'parallel'`）

**扇出-汇合即一个节点**：单游标的 PlanRunner 不变，并行藏在节点内部。`KernelNodeRunner` 遇到 `kind:'parallel'` → `runParallelNode`：
1. **并发**跑所有 branches（`Promise.all`，实际并发由 bridge `maxConcurrent` 限）；
2. **join 策略**（all/any/quorum）判定组是否通过；
3. **写分支合并**（L1+L2+L3c）：
   - **L1 不相交写域**：writer 分支声明 `writeScope`，`validatePlan` 静态拒绝「重叠且无 integrator」；`KernelBranchOps` 运行时把写域作为任务前缀约束分支；
   - **L2 改动交集预检**：`planMerge` 用各分支实际 `changedFiles` 算出干净合并集 + 冲突集；
   - **顺序合并**：干净的走 `mergeClean`（git，事务化、冲突即回滚不损坏）；冲突的走 **L3c integrator 角色**（`resolveAndMerge`，LLM 融合两边改动写回，再 discard 冲突分支）；
4. **扇入**：每个分支产出写 Blackboard `output`（持久、`readFor` 给下游）；
5. 聚合成一个 `OrchVerdict`（全过 → ok；join 不满足或合并失败 → fail + 纠偏，交回图）。

抽象：`BranchOps` 接口隔离 IO（`runParallelNode` 纯逻辑、stub 全测）；`KernelBranchOps` 是真实适配（dispatcher + `AutoWorktreeCoordinator`）。**安全降级**：合并冲突经事务回滚不损坏工作区，integrator 失败则浮现 fail（纠偏），绝不静默损坏。

**未开发（标 TODO，后续 PR）**
1. **节点级 hooks 装配**：把每节点 `hooks` 装配进该子 session 的 `KernelConfig.phaseHooks`（现仅主会话启动 hook 已接通；节点内相位 hook 待接，暂不暴露给 auto-orch）。
2. **闭环重规划**：在结构边界回调 Planner 改图，受 `replan-depth` 上限约束。
3. **并行运行时写域硬牢笼**：当前写域以任务前缀软约束 + L1 静态不相交保证；可加内核权限层的分支级路径白名单做硬强制。
4. **integrator 冲突解析的集成测试**：`KernelBranchOps.resolveAndMerge` 的真实 git 落地需 e2e 验证。
