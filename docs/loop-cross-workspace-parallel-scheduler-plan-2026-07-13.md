...
> 状态：主体协议已实现，全量测试与构建已通过；“外部路径权限闭包”按当前决策延期。
>
> 前置基线：`loop-single-workspace-closure.md`。本文只扩展单宿主上的多 workspace 并行，
> 不改变单 workspace 的 Charter、Artifact、Gate、Effect、Ledger 和恢复语义。
>
> 范围外：跨集群、多宿主协调、跨 workspace 工作流/数据依赖、Campaign、不受信第三方插件，
> 以及本阶段明确延期的 Charter 请求路径与操作员授权求交集。

### 0.1 2026-07-13 实施状态

| 能力 | 状态 |
|---|---|
| 稳定 Workspace ID、显式 fork 与实例绑定 | 已实现 |
| Worker/round/Distill Session namespace 与 metadata 校验 | 已实现；不导入旧 Loop 会话历史 |
| workspace scheduler registry/lease 与 duplicate identity fail-close | 已实现 |
| 宿主 round 公平准入、model-call 实际 API 准入与崩溃回收 | 已实现 |
| 声明式 exclusive/shared 外部资源租约 | 已实现 |
| EffectAdapter 跨进程 admission 与完整 scope 幂等身份 | 已实现 |
| Git remote/branch 租约与 non-fast-forward fail-stop | 已实现 |
| workspace-bound authenticated event/Human Gate | 已实现 |
| workspace-info/fork、schedulers、host-capacity CLI | 已实现 |
| Charter 请求路径 ∩ 操作员授权 | 延期；本次不改变既有路径授权行为 |
| 跨集群、跨宿主、跨 workspace 工作流 | 范围外 |
...
5. Worker 路径权限继续保持当前单 workspace 基线；请求路径与操作员授权求交集在本阶段延期，
   不作为跨 workspace 并行发布的前置条件。
...
Session ID 使用跨平台文件名安全字符；逻辑比较仍以 metadata 中的 Workspace ID/instance ID 为准，
不得通过拆解字符串恢复安全边界。
...
> 本节延期，当前实现不修改既有 `sandbox.writeAllowPaths` / `hostRequirements.writePaths`
> 行为。后续实施时仍以本节作为目标契约，但不得与本阶段跨 workspace 协调代码隐式耦合。
...
5. 本阶段不导入旧 Loop lineage；新 namespace 从 Ledger Capsule 建立，旧项目进展只通过
   Charter 明确引用的 workspace evidence 被看见；
6. 后续若单独实现 legacy lineage 导入，发现多个 workspace 竞争时必须拒绝，不得静默合并；
...
- 旧实例 Workspace ID 补绑（不导入旧 Loop Session）；
...
1. 路径授权交集测试随 G2 延期；既有单 workspace 路径权限回归必须保持通过；
2. exclusive 资源严格串行；
3. shared 资源遵守 maxConcurrent；
4. 多资源反序申请不会死锁；
5. 同 remote/branch 的 VCS publish 串行且拒绝非 fast-forward；
6. 同 adapter/credential profile 的调用遵守宿主级间隔和并发。
...
4. 全宿主并发严格受限且有公平性；
5. 外部 exclusive/shared 资源遵守声明的租约语义；
6. EffectAdapter admission、Git publish 和审批认证都绑定完整 workspace scope；
7. kill -9 后不丢成本、不重复已确认 round、不产生第二个逻辑外部任务；
8. 旧实例可安全补充 Workspace ID；旧 Loop Session 不导入且不跨 workspace 恢复；
9. 单 workspace 行为、恢复和性能与当前闭环基线保持 parity；
10. G2 路径授权交集保持显式延期，不得被误报为已实现。

达到以上标准后，才正式承诺：同一宿主上的多个 `loop-scheduler` 可以跨 workspace 并行运行，
并在状态、上下文、并发和外部资源层面互不干扰。外部路径授权交集须等 G2 单独完成后再追加承诺。
