...
charter:
  ...
  tripwires:   [{when: expr, then: {act: pivot|finalize|escalate, reason?, onResume?}}]  # 声明序即优先级；v3 判别联合，见 auto-orch-v3-route-redesign.md
  gates:       {name: {kind: schema|judge, ...}}
  seats:       {worker|judge|pivoter|finalizer: {context, tools, budget_per_round, prompt, inputs?}}
  health:      {staleWhen: expr}?   # progress.status 的 healthy|stale 规则（v3）
...
（WAKE→RECONCILE→MODE→CAPSULE→SEAT→GATE→METER→LEDGER→ROUTE；v3：绊线只在 ROUTE 求值一次，MODE 消费上一轮的 pivot 指令 + 内置预算守卫）。
...
### C10 升级与报告
attention/final 报告 = 代码模板渲染账本（final 报告叙事段可选由 finalizer 座位补写，v3）；
